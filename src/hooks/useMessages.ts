...
export function useMessages(): UseMessagesReturn {
    ...
    const filteredRecipients = (() => {
        const allRecipients = [
            { type: 'all', id: 'all', label: `👥 ${t('messages.allUsers')}`, searchText: t('messages.allUsers').toLowerCase() },
            ...users.map(u => {
                let label = '';
                let searchText = '';
                if (u.role === 'parent') {
                    const parent = u as Parent;
                    const children = users.filter(child => child.role === 'student' && parent.childrenIds?.includes(child.id));
            ...
            ...classes.map(c => ({ type: 'class', id: c.id, label: `🏫 ${c.name}`, searchText: `${c.name} ${t('common.classes')}`.toLowerCase() }))
        ];
    ...
    const selectedRecipientLabel = recipient ? filteredRecipients.find(r => r.id === recipient)?.label || recipientSearch : '';
    ...
    const handleSendMessage = () => {
        ...
        if (selectedRecipient?.type === 'class') {
            ...
            if (selectedClass) {
                const classStudents = users.filter(u => u.role === 'student' && (u as Student).classId === selectedClass.id);
                ...
                classStudents.forEach(student => {
                    sendMessage({
                        ...
                        receiverId: student.id, subject: `[${selectedClass.name}] ${subject}`,
                        content, read: false, type: 'group', attachments: attachmentUrls
                    });
                });
                if (classTeacher) {
                    sendMessage({
                        senderId: user.id, senderName: user.name, senderRole: user.role,
                        receiverId: classTeacher.id, subject: `[${selectedClass.name}] ${subject}`,
                        content, read: false, type: 'group', attachments: attachmentUrls
                    });
                }
            }
            ...
            sendMessage({
                senderId: user.id, senderName: user.name, senderRole: user.role,
                receiverId: recipient, subject, content, read: false,
                type: recipient === 'all' ? 'broadcast' : 'individual', attachments: attachmentUrls
            });
        }

        setIsComposeOpen(false);
        setRecipient(''); setRecipientSearch(''); setSubject(''); setContent(''); setAttachments([]);
    };
    ...
    return {
        selectedFolder, setSelectedFolder, filteredMessages,
        selectedMessage, setSelectedMessage, searchQuery, setSearchQuery,
        isMobile, mobileView, setMobileView,
        isComposeOpen, setIsComposeOpen, composeMode,
        recipient, setRecipient, recipientSearch, setRecipientSearch,
        showRecipientDropdown, setShowRecipientDropdown,
        subject, setSubject, content, setContent,
        attachments, setAttachments, fileInputRef,
        filteredRecipients, selectedRecipientLabel,
        handleArchiveMessage, handleComposeNew, handleReply, handleForward,
        handleFileSelect, removeAttachment, handleSendMessage,
        handleSelectMessage, handleDeleteMessage
    };
}
