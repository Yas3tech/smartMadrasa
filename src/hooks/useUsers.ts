...
export interface UseUsersReturn {
    ...
    birthDate: string; setBirthDate: (date: string) => void;
...
export function useUsers(): UseUsersReturn {
    ...
    const handleOpenNew = () => {
        ...
        setPhone(''); setBirthDate(''); setSelectedStudentId('');
        setIsModalOpen(true);
    };
    ...
    const handleEdit = (user: User) => {
        ...
        setPhone(user.phone || ''); setBirthDate(user.birthDate || '');
        if (user.role === 'parent') {
            ...
            for (const row of jsonData as any[]) {
                if (row.name && row.email && row.role) {
                    const newUser: any = {
                        ...
                        phone: row.phone, birthDate: row.birthDate,
                    ...
                    if (newUser.role === 'parent' && row.studentEmail) {
                        ...
                        if (student) newUser.childrenIds = [student.id];
                    }
                    addUser(newUser);
                    importedCount++;
                }
    ...
    return {
        ...
        students, filteredUsers, roleCounts, roleColors, fileInputRef,
        handleOpenNew, handleEdit, handleSave, handleDelete,
        handleDownloadTemplate, handleFileUpload, getRoleLabel
    };
}
