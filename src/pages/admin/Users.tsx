        ...
        <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
            <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                    <input type="file" ref={usr.fileInputRef} onChange={usr.handleFileUpload} accept=".xlsx,.xls,.csv" className="hidden" />
                    <Button variant="secondary" icon={FileDown} onClick={usr.handleDownloadTemplate}>{t('users.downloadTemplate')}</Button>
                    <Button variant="secondary" icon={FileSpreadsheet} onClick={() => usr.fileInputRef.current?.click()}>{t('users.importExcel')}</Button>
                    <Button variant="primary" icon={Plus} onClick={usr.handleOpenNew}>{t('users.newUser')}</Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {(['all', 'student', 'teacher', 'parent', 'director', 'superadmin'] as const).map(roleFilter => (
                    ...
                    >
                        <p className="text-xs text-gray-500 font-medium mb-1">{usr.getRoleLabel(roleFilter)}</p>
                        ...
                        <tbody>
                            {usr.filteredUsers.map(user => (
                                <tr key={user.id} className="border-b border-gray-100 hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${usr.roleColors[user.role]}`}>
                                            {t(`roles.${user.role}`)}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className={`flex ${isRTL ? 'justify-start' : 'justify-end'} gap-2`}>
                                            <button onClick={() => usr.handleEdit(user)} className="p-2 text-gray-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors">
                                                <Edit2 size={18} />
                                            </button>
                                            <button onClick={() => usr.handleDelete(user.id, user.role)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" disabled={user.id === currentUser?.id}>
                                                <Trash2 size={18} />
                                            </button>
                                        </div>
                                    </td>
            ...
            <Modal isOpen={usr.isModalOpen} onClose={() => usr.setIsModalOpen(false)}>
                <div className="p-6">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button onClick={() => usr.setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600">
                            <X size={24} />
                        </button>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <Input label={t('users.fullName')} value={usr.name} onChange={(e) => usr.setName(e.target.value)} placeholder="Jean Dupont" />
                        <Input label={t('users.email')} type="email" value={usr.email} onChange={(e) => usr.setEmail(e.target.value)} placeholder="jean.dupont@school.ma" />

                        {(usr.role === 'teacher' || usr.role === 'parent') && (
                            <Input label={t('users.phone')} type="tel" value={usr.phone} onChange={(e) => usr.setPhone(e.target.value)} placeholder="+212 6..." />
                        )}

                        {usr.role === 'student' && (
                            <Input label={t('users.birthDate')} type="date" value={usr.birthDate} onChange={(e) => usr.setBirthDate(e.target.value)} />
                        )}
                        ...
                        <div className="flex justify-end gap-3 pt-4">
                            <Button variant="secondary" onClick={() => usr.setIsModalOpen(false)}>{t('common.cancel')}</Button>
