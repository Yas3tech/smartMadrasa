...
import {
    Search, PenSquare, Inbox, Send, Archive, Trash2,
    Reply as ReplyIcon, Forward as ForwardIcon, Filter,
    X, Paperclip, File, ChevronLeft
} from 'lucide-react';
    ...
    return (
        <div className="h-[calc(100vh-6rem)] flex flex-col">
            <Card className="flex-1 flex overflow-hidden !p-0 border-0 shadow-xl">
                ...
                <div className={`${msg.isMobile
                    ? (msg.mobileView === 'folders' ? 'flex-1 w-full bg-white dark:bg-slate-800' : 'hidden')
                    ...
                    <div className="p-4">
                        ...
                        <Button className="w-full justify-start mb-6" icon={PenSquare} onClick={msg.handleComposeNew}>
                ...
                <div className={`${msg.isMobile
                    ? (msg.mobileView === 'list' ? 'flex-1 w-full bg-white dark:bg-slate-900' : 'hidden')
                    ...
                    <div className="p-4 border-b border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800">
                        <div className="flex items-center gap-2 mb-2">
                            {msg.isMobile && (
                                <button onClick={() => msg.setMobileView('folders')} className="p-1 mr-2 -ml-2 text-gray-500">
                                    <ChevronLeft size={24} />
                                </button>
                            )}
                            <div>
                                ...
                                <p className="text-xs text-gray-500 dark:text-slate-400">{msg.filteredMessages.length} {t('messages.message').toLowerCase()}{msg.filteredMessages.length !== 1 ? 's' : ''}</p>
                            </div>
                        </div>
                            ...
                            >
                                <div className="flex justify-between items-start mb-1">
                                    <div className="flex items-center gap-2">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white ${msg.selectedMessage?.id === message.id ? 'bg-orange-500' : 'bg-gray-400'}`}>
                                            {message.senderName.charAt(0)}
                                        </div>
                                        <span className={`text-sm font-semibold ${msg.selectedMessage?.id === message.id ? 'text-orange-900' : 'text-gray-900'}`}>
                                            {message.senderName}
                                        </span>
                                    </div>
                                ...
                                <h3 className={`text-sm mb-1 ${!message.read ? 'font-bold text-gray-900' : 'text-gray-700'}`}>
                                    {message.subject}
                                </h3>
                                ...
                                {message.attachments && message.attachments.length > 0 && (
                                    <div className="mt-2 flex items-center gap-1 text-xs text-gray-400">
                                        <Paperclip size={12} />
                                        <span>{message.attachments.length} {t('messages.attachments').toLowerCase()}</span>
                                    </div>
                                )}
                            </div>
                        ))}
                ...
                <div className={`${msg.isMobile
                    ? (msg.mobileView === 'detail' ? 'flex-1 w-full bg-white dark:bg-slate-800 flex flex-col' : 'hidden')
                    ...
                    {msg.selectedMessage ? (
                        <>
                            ...
                            <div className="p-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
                                <div className="flex items-center gap-2">
                                    {msg.isMobile && (
                                        <button onClick={() => msg.setMobileView('list')} className="p-2 mr-2 -ml-2 text-gray-500 hover:bg-gray-100 rounded-lg">
                                            <ChevronLeft size={24} />
                                        </button>
                                    )}
                                    <div className="flex gap-2">
                                        <button className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-lg" onClick={() => msg.handleReply(msg.selectedMessage!)} title={t('messages.reply')}>
                                            <ReplyIcon size={18} />
                                        </button>
                                        <button className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-lg" onClick={() => msg.handleForward(msg.selectedMessage!)} title={t('messages.forward')}>
                                            <ForwardIcon size={18} />
                                        </button>
                                        <button className={`p-2 transition-colors rounded-lg ${msg.selectedMessage.archived ? 'text-gray-400 hover:text-blue-600 hover:bg-blue-50' : 'text-gray-400 hover:text-orange-600 hover:bg-orange-50'}`} onClick={() => msg.handleArchiveMessage(msg.selectedMessage!.id)} title={msg.selectedMessage.archived ? "Désarchiver" : t('messages.archive')}>
                                            <Archive size={18} />
                                        </button>
                                        <button className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg" onClick={() => msg.handleDeleteMessage(msg.selectedMessage!.id)} title={t('common.delete')}>
                                            <Trash2 size={18} />
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="flex-1 p-8 overflow-y-auto">
                                <div className="flex justify-between items-start mb-8">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h2 className="text-lg font-bold text-gray-900 dark:text-white">{msg.selectedMessage.senderName}</h2>
                                            <p className="text-sm text-gray-500 dark:text-slate-400">{msg.selectedMessage.senderRole}@smartmadrasa.com</p>
                                        </div>
                                    </div>
                                ...
                                <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">{msg.selectedMessage.subject}</h1>
                                    ...
                                    <div className="border-t pt-6">
                                        ...
                                        <div className="flex flex-wrap gap-3">
                                            {msg.selectedMessage.attachments.map((url, index) => (
                                                <a key={index} href={url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 transition-colors">
                                                    <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-orange-500 shadow-sm">
                                                        <File size={20} />
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-medium text-gray-900">{t('messages.attachments')} {index + 1}</p>
                ...
                <div className="p-6">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                            {msg.composeMode === 'reply' ? t('messages.reply') : msg.composeMode === 'forward' ? t('messages.forward') : t('messages.newMessage')}
                        </h2>
                        <button onClick={() => msg.setIsComposeOpen(false)} className="text-gray-400 hover:text-gray-600">
                            <X size={24} />
                        </button>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="relative">
                            ...
                            <input
                                ...
                                onChange={(e) => { msg.setRecipientSearch(e.target.value); msg.setRecipient(''); msg.setShowRecipientDropdown(true); }}
                            ...
                            {msg.showRecipientDropdown && !msg.recipient && msg.filteredRecipients.length > 0 && (
                                <div className="absolute z-50 w-full mt-1 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                                    {msg.filteredRecipients.map((item) => (
                                        <button key={item.id} type="button" onClick={() => { msg.setRecipient(item.id); msg.setRecipientSearch(''); msg.setShowRecipientDropdown(false); }} className="w-full px-4 py-3 text-left hover:bg-orange-50 dark:hover:bg-slate-600 transition-colors flex items-center gap-2 border-b border-gray-100 dark:border-slate-600 last:border-0">
                        ...
                        <Input label={t('messages.subject')} value={msg.subject} onChange={(e) => msg.setSubject(e.target.value)} placeholder={t('messages.subjectPlaceholder')} />

                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('messages.message')}</label>
                            <textarea value={msg.content} onChange={(e) => msg.setContent(e.target.value)} rows={8} className="w-full px-4 py-2 rounded-xl border border-gray-200 focus:ring-2 focus:ring-orange-100 focus:border-orange-500 outline-none resize-none" placeholder={t('messages.messagePlaceholder')} />
                        </div>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <button onClick={() => msg.fileInputRef.current?.click()} className="text-sm text-orange-600 hover:text-orange-700 font-medium flex items-center gap-1">
                                    <Paperclip size={16} />
                                    {t('messages.addAttachment')}
                                </button>
                                <input type="file" ref={msg.fileInputRef} onChange={msg.handleFileSelect} className="hidden" multiple />
                            </div>
                                ...
                                <div className="space-y-2">
                                    {msg.attachments.map((file, index) => (
                                        <div key={index} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-slate-700 rounded-lg border border-gray-200 dark:border-slate-600">
                                            ...
                                            <button onClick={() => msg.removeAttachment(index)} className="text-gray-400 hover:text-red-500 p-1">
                                                <X size={16} />
                                            </button>
                                        </div>
                                    ))}
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t">
                            <Button variant="secondary" onClick={() => msg.setIsComposeOpen(false)}>{t('common.cancel')}</Button>
                            <Button variant="primary" onClick={msg.handleSendMessage} icon={Send}>{t('messages.send')}</Button>
                        </div>
                    </div>
                </div>
            </Modal>
