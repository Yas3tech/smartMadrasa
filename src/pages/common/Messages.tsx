...
const Messages = () => {
  ...
  const handleSendMessage = useThrottle(useCallback(async () => {
    ...
    try {
      ...
      // ⚡ Bolt: Pre-compute maps to replace O(N^2) loops with O(1) map lookups
      const classesMap = new Map(classes.map(c => [c.id, c]));
      const usersMap = new Map(users.map(u => [u.id, u]));
      const studentsByClassMap = new Map<string, Student[]>();

      users.forEach(u => {
        if (u.role === 'student') {
          const student = u as Student;
          if (student.classId) {
            if (!studentsByClassMap.has(student.classId)) {
              studentsByClassMap.set(student.classId, []);
            }
            studentsByClassMap.get(student.classId)!.push(student);
          }
        }
      });
      ...
      const allOutbound = recipients.map(async (recipientId) => {
        ...
        if (selectedRecipient?.type === 'class') {
          ...
          if (selectedClass) {
            const classStudents = studentsByClassMap.get(selectedClass.id) || [];
