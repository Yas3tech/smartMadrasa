...
import { useAcademics, usePerformance } from '../../context/hooks';
...
const HomeworkPage = () => {
  ...
  const teacherClasses =
    user.role === 'teacher' ? classes.filter((c) => c.teacherId === user.id) : classes;
  ...
  const targetClassId =
    hw.selectedChild?.classId ||
    (user.role === 'student' ? (user as { classId?: string }).classId : null);
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            >
              {hw.showPastHomework
                ? t('homework.hidePastHomework')
                : t('homework.showPastHomework')}
  ...
  return (
    <div className="space-y-6">
      ...
      {user.role === 'parent' && (
        <StudentSelector onSelect={hw.setSelectedChild} selectedStudentId={hw.selectedChild?.id} />
      )}
      ...
      <SubmitHomeworkModal
        ...
        formatFileSize={hw.formatFileSize}
        existingSubmission={
          hw.selectedHomework ? hw.studentSubmissions.get(hw.selectedHomework.id) : undefined
        }
      />
    </div>
  );
