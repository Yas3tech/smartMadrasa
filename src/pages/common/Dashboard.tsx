        ...
        <div className="space-y-6 animate-fadeIn">
            ...
            {(user.role === 'student' || user.role === 'parent') && (
                <StudentDashboard
                    ...
                    myAvg={dash.myAvg}
                    mySubjectPerformance={dash.mySubjectPerformance}
            ...
            <Modal isOpen={dash.showHomeworkModal} onClose={() => { dash.setShowHomeworkModal(false); dash.setSelectedHomework(null); }}>
                {dash.selectedHomework && (
                    <div className="p-6">
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <button onClick={() => { dash.setShowHomeworkModal(false); dash.setSelectedHomework(null); }} className="text-gray-400 hover:text-gray-600">
                                <X size={24} />
                            </button>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-lg">
                                ...
                                <div>
                                    ...
                                    <p className="font-semibold text-gray-900">
                                        {new Date(dash.selectedHomework.dueDate).toLocaleDateString(i18n.language, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="p-4 bg-gray-50 rounded-lg">
                                ...
                                <p className="text-gray-800 whitespace-pre-wrap">{dash.selectedHomework.description || t('homework.noDescription')}</p>
                            </div>

                            {dash.selectedHomework.assignedBy && (
                                <div className="flex items-center gap-2 text-sm text-gray-500">
                                    <GraduationCap size={16} />
                                    <span>{t('homework.assignedBy')}: {dash.selectedHomework.assignedBy}</span>
                                </div>
                            )}
                            ...
                            {dash.selectedHomework.maxGrade && (
                                <div className="flex items-center gap-2 text-sm text-gray-500">
                                    <TrendingUp size={16} />
                                    <span>{t('homework.maxGrade')}: {dash.selectedHomework.maxGrade} pts</span>
                                </div>
                            )}
                        </div>

                        <div className="mt-6 pt-4 border-t">
                            <Button variant="primary" className="w-full" onClick={() => { dash.setShowHomeworkModal(false); navigate('/homework'); }}>
                                {t('homework.submit')}
                            </Button>
                        </div>
                    </div>
