...
const TeacherBulletinGrades: React.FC = () => {
  ...
  const studentValidatedCounts = useMemo(() => {
    ...
    for (const comment of classComments) {
      if (comment.periodId === selectedPeriod && comment.isValidated) {
        counts.set(comment.studentId, (counts.get(comment.studentId) || 0) + 1);
      }
    }
    return counts;
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6">
          <div className="flex items-center gap-2 mb-4">
            <button
              ...
              className="p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg text-gray-700 dark:text-slate-300"
            >
              <ArrowLeft size={20} />
            </button>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg text-gray-700 dark:text-slate-300 flex-shrink-0"
                >
                  <ArrowLeft size={20} />
                </button>
                <div className="min-w-0">
                  <h2 className="text-xl font-semibold dark:text-white truncate">
                    {selectedClassData?.name} - {t('bulletinGrades.overview')}
                  </h2>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                <div className="bg-indigo-50 dark:bg-indigo-900/20 px-4 py-2 rounded-lg flex justify-between items-center sm:block text-right">
                  <span className="text-sm text-gray-500 sm:hidden">
                    {t('bulletinGrades.validationProgress')}
                  </span>
                  <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {classStudents.map((student) => {
                // Reimplementing minimal logic for UI display using O(1) map lookup:
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-gray-600 dark:text-slate-400 truncate">
                    {selectedPeriodData?.name} - {selectedPeriodData?.academicYear}
                  </p>
                </div>
              </div>
              <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-lg p-6 text-white w-full lg:w-auto">
                <div className="flex items-center justify-between lg:block">
                  <span className="text-indigo-100 text-sm lg:mb-1 block">
                    {t('bulletinGrades.generalAverage')}
                  </span>
            ...
            {studentCourseAverages.length === 0 ? (
              <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-12 text-center">
                ...
                <p className="text-gray-500 dark:text-slate-400">
                  {t('bulletinGrades.noGradesForPeriod')}
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-slate-400">
                          {gradeCount}{' '}
                          {gradeCount > 1 ? t('bulletinGrades.grades') : t('bulletinGrades.grade')}
                        </p>
                      </div>
                    </div>
                  ...
                  {existingComment && existingComment.isValidated ? (
                    <div className="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800 rounded-lg p-4">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                        {t('bulletinGrades.validatedOn', {
                          date: new Date(existingComment.validationDate!).toLocaleDateString(
                            locale
                          ),
                        })}
                      </p>
                    ...
                    <div className="space-y-3">
                      <label className="block text-sm font-medium text-gray-700">
                        ...
                        {existingComment
                          ? t('bulletinGrades.editComment')
                          : t('bulletinGrades.addComment')}
                      </label>

                      <textarea
                        value={
                          comments[course.id] !== undefined
                            ? comments[course.id]
