...
const BulletinPreview: React.FC<BulletinPreviewProps> = ({
    ...
    const courseData = courses.map(course => {
        ...
        const average = periodGrades.length > 0
            ? periodGrades.reduce((acc, g) => acc + g.score, 0) / periodGrades.length
            : null;

        const comment = comments.find(c => c.courseId === course.id && c.periodId === period.id);
    ...
    const overallAverage = courseData.length > 0
        ? courseData.reduce((acc, c) => acc + (c.average || 0), 0) / courseData.filter(c => c.average !== null).length
        : 0;
                    ...
                    <div className="bg-white shadow-lg p-8 mx-auto max-w-[210mm] min-h-[297mm]">
                        ...
                        <div className="grid grid-cols-2 gap-8 mb-8">
                            <div className="bg-gray-50 p-4 rounded-lg">
                                <h3 className="font-bold text-gray-700 mb-2 border-b pb-1">{t('bulletin.student')}</h3>
                                <p><span className="text-gray-500">{t('bulletin.studentName')} :</span> <span className="font-medium">{student.name}</span></p>
                                <p><span className="text-gray-500">{t('bulletin.studentClass')} :</span> <span className="font-medium">{student.className || 'N/A'}</span></p>
                            </div>
                            <div className="bg-gray-50 p-4 rounded-lg">
                                <h3 className="font-bold text-gray-700 mb-2 border-b pb-1">{t('bulletin.period')}</h3>
                                <p><span className="text-gray-500">{t('bulletin.periodName')} :</span> <span className="font-medium">{period.name}</span></p>
                                <p><span className="text-gray-500">{t('bulletin.periodYear')} :</span> <span className="font-medium">{period.academicYear}</span></p>
                            </div>
                        </div>
                        ...
                        <table className="w-full mb-8 border-collapse">
                            <thead>
                                <tr className="bg-indigo-600 text-white">
                                    <th className="p-3 text-left border border-indigo-600">{t('bulletin.subject')}</th>
                                    <th className="p-3 text-center border border-indigo-600 w-32">{t('bulletin.average')}</th>
                                    <th className="p-3 text-left border border-indigo-600">{t('bulletin.comment')}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {courseData.map((item, index) => (
                                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                        <td className="p-3 border border-gray-200 font-medium">{item.course.subject}</td>
                        ...
                        <div className="flex justify-between items-start mt-8">
                            <div className="w-1/2">
                                ...
                                <ul className="text-sm space-y-1 text-gray-600">
                                    <li>• {t('bulletin.justified')} : {absences.justified}</li>
                                    <li>• {t('bulletin.unjustified')} : {absences.unjustified}</li>
                                    <li>• {t('bulletin.late')} : {absences.late}</li>
                                </ul>
                            </div>
                            <div className="w-64 bg-gray-100 p-4 rounded-lg border border-gray-200 text-center">
                                ...
                                <p className="text-3xl font-bold text-indigo-600">{overallAverage.toFixed(2)} <span className="text-sm text-gray-400">/ 20</span></p>
                            </div>
                        </div>
