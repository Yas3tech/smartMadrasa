...
import {
    Download, Search, Upload, Pencil, X, Check, Plus, ArrowLeft, User, BookOpen, Users
} from 'lucide-react';
            ...
            <div className="lg:hidden -mx-4 -mt-6 bg-gradient-to-r from-orange-500 to-orange-600 p-6 rounded-b-[2rem] shadow-lg mb-6 text-white">
                <div className="flex justify-between items-center mb-4">
                    <h1 className="text-2xl font-bold flex items-center gap-2">
                        ...
                        {tg.selectedClass ? `${tg.selectedClass.name} - ` : ''}{t('grades.management')}
                    </h1>
                    <button
            ...
            <div className="hidden lg:flex justify-between items-center">
                ...
                <div className="flex gap-2">
                    <Button
                        ...
                        icon={Download}
                        onClick={() => {
                            if (tg.selectedStudentData && tg.selectedStudentId) {
                                const stats = calculateStudentStats(tg.selectedStudentId);
                                generateGradeReport(
                                    tg.selectedStudentData,
                                    tg.studentGrades,
                                    stats,
                                    t,
                                    i18n.language
                        ...
                        disabled={!tg.selectedStudentId}
                    >
                        {t('grades.exportReport')}
                    </Button>
                    <Button
                        variant="primary"
                        icon={Plus}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {tg.teacherClasses.map((cls) => (
                        ...
                        >
                            ...
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">{cls.name}</h3>
            ...
            {tg.selectedClassId && !tg.viewMode && (
                <div className="space-y-6 animate-fadeIn">
                    <div className="flex items-center gap-3">
                        <button onClick={tg.goBack} className="p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg transition-colors">
                            <ArrowLeft className="text-gray-600 dark:text-slate-300" size={20} />
                        </button>
                        <div>
                            <h2 className="text-xl font-bold text-gray-900 dark:text-white">{tg.selectedClass?.name}</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        >
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{t('grades.viewByStudent')}</h3>
                                <p className="text-sm text-gray-500 dark:text-slate-400">{t('grades.viewByStudentDesc')}</p>
                            </div>
                        </button>
                        ...
                        >
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{t('grades.viewBySubject')}</h3>
                                <p className="text-sm text-gray-500 dark:text-slate-400">{t('grades.viewBySubjectDesc')}</p>
                            </div>
                        </button>
                    </div>
                </div>
                ...
                <div className="space-y-6 animate-fadeIn">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-gray-900 dark:text-white">{tg.selectedStudentData ? tg.selectedStudentData.name : t('grades.studentList')}</h2>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-100 dark:border-slate-700">
                                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-slate-300">{t('grades.subject')}</th>
                                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-slate-300">{t('grades.score')}</th>
                                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-slate-300">{t('grades.type')}</th>
                                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-slate-300">{t('grades.date')}</th>
                                            <th className="px-6 py-4 text-right text-sm font-semibold text-gray-600 dark:text-slate-300">{t('common.actions')}</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                        {tg.studentGrades.map(grade => (
                                            <tr key={grade.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    {tg.editingGrade?.id === grade.id ? (
                                                        ...
                                                        <span className={`font-bold ${grade.score < grade.maxScore / 2 ? 'text-red-500' : 'text-gray-900 dark:text-white'}`}>
                                                            {grade.score}/{grade.maxScore}
                                                        </span>
                                                    )}
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className={`px-2 py-1 rounded text-xs font-medium ${grade.type === 'exam' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' :
                                                        grade.type === 'homework' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' :
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    {tg.editingGrade?.id === grade.id ? (
                                                        <div className="flex justify-end gap-2">
                                                            <button onClick={tg.handleUpdateGrade} className="p-1 text-green-600 hover:bg-green-50 rounded">
                                                                <Check size={18} />
                                                            </button>
                                                            <button onClick={tg.cancelEditing} className="p-1 text-red-600 hover:bg-red-50 rounded">
                                                                <X size={18} />
                                                            </button>
                                                        </div>
                                                    ) : (
                                                        <button onClick={() => tg.startEditingGrade(grade)} className="p-1 text-gray-400 hover:text-indigo-600 transition-colors">
                                                            <Pencil size={18} />
                                                        </button>
                                                    )}
                                                </td>
            ...
            {tg.viewMode === 'bySubject' && (
                <div className="space-y-6 animate-fadeIn">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-gray-900 dark:text-white">{tg.selectedSubject || t('grades.selectSubject')}</h2>
                            <p className="text-gray-500 dark:text-gray-400">{tg.selectedClass?.name}</p>
                        </div>
                    </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden">
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    <thead>
                                        <tr className="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-100 dark:border-slate-700">
                                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-slate-300">{t('grades.student')}</th>
                                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-slate-300">{t('grades.score')}</th>
                                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-slate-300">{t('grades.type')}</th>
                                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-600 dark:text-slate-300">{t('grades.date')}</th>
                                            <th className="px-6 py-4 text-right text-sm font-semibold text-gray-600 dark:text-slate-300">{t('common.actions')}</th>
