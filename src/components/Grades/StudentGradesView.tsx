...
const StudentGradesView = () => {
  ...
  // Pre-compute teacher and course maps to avoid O(N) lookups in render loop
  ...
  const courseMap = useMemo(() => {
    ...
    courses.forEach((c) => map.set(`${c.classId}::${c.subject}`, c.teacherName));
    return map;
  }, [courses]);
  ...
  const getTeacherName = (grade: Grade) => {
    if (grade.teacherId) {
      ...
      if (teacherName !== undefined) return teacherName;
    }
    return courseMap.get(`${grade.classId}::${grade.subject}`);
  };
