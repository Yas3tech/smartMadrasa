...
const StudentGradesView = () => {
  ...
  // Pre-compute teacher name map for O(1) lookups
  // Iterate backwards to preserve the behavior of .find() (returns the first match)
  const teacherMap = useMemo(() => {
    const map = new Map<string, string>();
    for (let i = users.length - 1; i >= 0; i--) {
      map.set(users[i].id, users[i].name);
    }
    return map;
  }, [users]);

  // Pre-compute course teacher map using a composite key
  // Iterate backwards to preserve the behavior of .find() (returns the first match)
  const courseTeacherMap = useMemo(() => {
    ...
    for (let i = courses.length - 1; i >= 0; i--) {
      const c = courses[i];
      if (c.teacherName) {
        map.set(`${c.classId}::${c.subject}`, c.teacherName);
      }
    }
    return map;
  }, [courses]);
  ...
  const getTeacherName = (grade: Grade) => {
    if (grade.teacherId) {
      const teacherName = teacherMap.get(grade.teacherId);
      if (teacherName) return teacherName;
    }
    return courseTeacherMap.get(`${grade.classId}::${grade.subject}`);
  };
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">{t('grades.academicReport')}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/30 dark:to-orange-800/20 border-orange-200 dark:border-orange-800">
          ...
          <p className="text-4xl font-bold text-gray-900 dark:text-white mb-2">{stats.avgGrade}%</p>
          <div className="flex items-center gap-1 text-sm">
            {stats.avgGrade >= 80 ? (
              ...
              <span className="text-orange-600 dark:text-orange-400 font-medium">
                {t('grades.needsImprovement')}
              </span>
            )}
          </div>
        </Card>

        <Card className="p-6 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/20 border-blue-200 dark:border-blue-800">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2 bg-white dark:bg-slate-700 rounded-lg">
        ...
        <Card className="p-6 bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/30 dark:to-green-800/20 border-green-200 dark:border-green-800">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2 bg-white dark:bg-slate-700 rounded-lg">
              <BarChart3 className="text-green-600 dark:text-green-400" size={24} />
            </div>
            <h3 className="font-bold text-gray-900 dark:text-white">{t('grades.evaluations')}</h3>
          </div>
          <p className="text-4xl font-bold text-gray-900 dark:text-white mb-2">
            {stats.totalGrades}
          </p>
      ...
      <div>
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6">
          {t('grades.recentResults')}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {myGrades.map((grade) => (
            <GradeCard key={grade.id} grade={grade} teacherName={getTeacherName(grade)} />
          ))}
          {myGrades.length === 0 && (
            <div className="col-span-full text-center py-12 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700">
              <FileText size={48} className="mx-auto mb-4 text-gray-300 dark:text-gray-500" />
