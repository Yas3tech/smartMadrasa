...
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, GraduationCap, BookOpen, Trash2, Plus } from 'lucide-react';
import type { Course } from '../../types';

interface ScheduleMobileProps {
    schedule: any; // Type as 'any' for now since the wrapper is a complex object. In a real app we'd define this precisely.
    user: any;
    t: any;
    i18n: any;
}
    ...
    return (
        <div className="lg:hidden -mx-4 -mt-4 md:-mx-6 md:-mt-6 bg-gray-50 dark:bg-slate-950 min-h-[calc(100vh-4rem)]">
            ...
            <div className="px-4 space-y-4 pb-24">
                ...
                {/* Mobile Courses — groups overlapping courses side-by-side */}
                <div className="mb-6">
                    {schedule.mobileCourses.length > 0 ? (
                        <div className="space-y-3">
                            {(() => {
                                // Group overlapping courses by time range overlap
                                const groups: Course[][] = [];
                                const sorted = [...schedule.mobileCourses] as Course[];
                                let i = 0;
                                while (i < sorted.length) {
                                    const group: Course[] = [sorted[i]];
                                    const groupEndMin = (() => { const [h, m] = sorted[i].endTime.split(':').map(Number); return h * 60 + m; })();
