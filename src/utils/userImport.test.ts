...
describe('userImport', () => {
  ...
  it('validates multiple comma-separated student emails', () => {
    const reviewedRows = validateUserImportRows(
      ...
      {
        existingUsers: [
          { id: '1', name: 'Child 1', email: 'child1@school.ma', role: 'student' } as unknown as import('../types').User,
        ],
        existingStudents: [
          { id: '1', name: 'Child 1', email: 'child1@school.ma', role: 'student' } as unknown as import('../types').Student,
        ],
        canImportSuperadmin: false,
      }
    );
