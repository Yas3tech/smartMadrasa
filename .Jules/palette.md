...
## 2025-06-21 - Schedule Icon-only Buttons Accessibility
**Learning:** Icon-only buttons used for navigation and actions in `ScheduleMobile` and `ScheduleDesktop` (e.g., `ChevronLeft`, `Trash2`, `Plus`) lacked explicit `aria-label` attributes. Without these, screen readers announce "button" with no context, confusing users.
**Action:** Always verify that every instance of an icon-only button—whether a native `<button>` or custom `<Button>` component—includes a descriptive `aria-label` utilizing standard `t('common.*')` translation keys for full accessibility.
