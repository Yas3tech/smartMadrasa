...
## 2025-06-05 - Icon-only buttons accessibility in TeacherGradesView

**Learning:** Important icon-only action buttons in tables (Edit, Save, Cancel, Bulk Upload) were missing accessible names, and their focus states were visually weak or missing.
**Action:** Add `aria-label` and `title` to these buttons using translation strings, and add explicit `focus-visible:ring-2 focus-visible:ring-{color}-500` styles to ensure keyboard users have clear visual feedback when navigating tables.
