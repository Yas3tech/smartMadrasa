...
async function fixField(collectionName, docId, field, currentValue, correctValue, label) {
  totalProblems++;
  console.log(
    `  [FIX] ${collectionName}/${docId} → ${field}: "${currentValue}" → "${correctValue}"  (${label})`
  );
  if (!DRY_RUN) {
    await db
      .collection(collectionName)
      .doc(docId)
      .update({ [field]: correctValue });
    totalFixed++;
  }
}
...
async function fixUserDocIds(authByEmail) {
  ...
  for (const docSnap of snap.docs) {
    ...
    if (!email) {
      console.log(`  [SKIP] No email for doc ${fsId}`);
      skipped++;
      continue;
    }
    ...
    if (!authUser) {
      console.log(`  [SKIP] No Auth account for "${email}"`);
      skipped++;
      continue;
    }
...
async function main() {
  ...
  const authByEmail = new Map(
    allAuthUsers.filter((u) => u.email).map((u) => [u.email.toLowerCase(), u])
  );
