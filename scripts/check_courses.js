...
async function checkCourses() {
  ...
  for (const classDoc of classesSnap.docs) {
    ...
    for (const courseDoc of coursesSnap.docs) {
      ...
      console.log(
        `    Course [${courseDoc.id}]: "${courseData.name}" | teacherId: ${courseData.teacherId}`
      );

      // Check if teacherId matches any user doc's id
      const matchingUser = usersSnap.docs.find(
        (u) => u.data().id === courseData.teacherId || u.id === courseData.teacherId
      );
      if (matchingUser) {
        const realUid = matchingUser.id;
        if (courseData.teacherId !== realUid) {
          console.log(
            `      ⚠️  teacherId mismatch! Stored: ${courseData.teacherId}, should be: ${realUid}`
          );
        ...
        console.log(
          `      ⚠️  Could not find matching user for teacherId: ${courseData.teacherId}`
        );
      }
    }
  }
