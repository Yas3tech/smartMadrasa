...
async function migrate() {
  ...
  if (!email || !password) {
    console.error(
      'Error: Please set ADMIN_EMAIL and ADMIN_PASSWORD in .env or environment variables to authenticate as an admin.'
    );
    process.exit(1);
  }
  ...
  for (const parent of parents) {
    ...
    if (needsUpdate) {
      console.log(
        `Updating parent ${parent.id} (${parent.name}): relatedClassIds = [${newRelatedClassIds.join(', ')}]`
      );
      try {
