...
async function main() {
  ...
  const emailToAuthUid = new Map(
    allAuthUsers.filter((u) => u.email).map((u) => [u.email.toLowerCase(), u.uid])
  );
  ...
  for (const classDoc of classesSnap.docs) {
    ...
    if (byAuthUid.has(currentTeacherId)) {
      console.log(
        `[OK ] Class "${cls.name}" - teacherId "${currentTeacherId}" is already the Auth UID`
      );
      okCount++;
      continue;
    }
    ...
    if (!teacherDoc || !teacherDoc.email) {
      console.log(
        `[????] Class "${cls.name}" - teacherId "${currentTeacherId}" not found in users`
      );
      skippedCount++;
      continue;
    }
    ...
    if (!correctUid) {
      console.log(
        `[WARN] Class "${cls.name}" - teacher email "${teacherDoc.email}" has no Auth account`
      );
      skippedCount++;
      continue;
    }
    ...
    console.log(
      `[FIX] Class "${cls.name}": teacherId "${currentTeacherId}" → "${correctUid}" (${teacherDoc.email})`
    );
    if (!DRY_RUN) {
