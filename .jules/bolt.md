...
## 2026-05-31 - [CI Failure Resolution - Vulnerable Dependencies]
**Learning:** Found CI failing on `quality-and-security` step due to high severity vulnerabilities reported by `pnpm audit:deps` in several transient dependencies (e.g. `picomatch`, `protobufjs`, `vite`, `jspdf`, `flatted`).
**Action:** Resolved by running `pnpm update` or `pnpm install` with explicit updated versions (e.g. `pnpm install jspdf@^4.2.1 protobufjs@^7.5.6 ...`), then used `pnpm.overrides` block in `package.json` for nested dependencies to force safe versions. Always run `pnpm audit --audit-level high` to confirm security fixes.
