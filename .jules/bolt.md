...
## 2025-03-03 - [O(S*K) in TeacherBulletinGrades]
**Learning:** Found a nested loop O(S * K) rendering bottleneck in `TeacherBulletinGrades.tsx` where `.filter()` on `classComments` was called inside `classStudents.map()` during React rendering.
**Action:** Extracted the `.filter()` into a single `O(K)` pass over `classComments` inside an IIFE (Immediately Invoked Function Expression) to generate a lookup `Map` of counts, bringing the overall rendering complexity down to `O(S + K)`. Pre-compute lookup objects *before* array mapping.
