...
## 2026-03-03 - [Nested Loops O(N log N) Date Instantiation Optimization]
**Learning:** Found a severe performance overhead in `NotificationBell.tsx` where date strings were repeatedly parsed `new Date(string)` inside deeply nested `.filter()` and `.sort()` operations for `messages`, `grades`, and `events` upon every component render. Since UI state changes (like opening the bell dropdown) trigger re-renders, the CPU would repetitively process the unmemoized filtering and O(N log N) parsing.
**Action:** Wrapped the entire `generateNotifications` array construction in a `useMemo` block to memoize the final array. Replaced inline `new Date(a).getTime()` calls with a Schwartzian transform (map-sort-map), mapping objects to `{ original: object, parsed: Date.parse(object.date) }` before sorting. This avoids instantiating unused `Date` objects and restricts parsing complexity to O(N). Always use Schwartzian transforms when sorting dates to minimize object creation overhead.
