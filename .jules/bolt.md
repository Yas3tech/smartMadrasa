...
## 2025-05-15 - [O(N²) Rendering Optimization in Classes]
**Learning:** Found a severe O(C * U) and O(C * S) performance bottleneck in `Classes.tsx` where an `Array.prototype.find()` lookup over users and an `Array.prototype.filter()` over students were executed inside a render loop mapping over classes.
**Action:** Used `useMemo` to pre-compute a `teachersMap` (O(U)) and a `classStudentsMap` (O(S)). This flattens the rendering complexity to O(C + U + S) by transforming nested lookups into O(1) reads.
