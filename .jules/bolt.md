...
## 2026-03-04 - [O(N²) Array Lookup Optimization in Profile Rendering]
**Learning:** Using `Array.prototype.find()` inside array `.map()` calls or conditional render blocks (e.g., retrieving class information for each child in a parent's profile) creates an O(N*M) performance bottleneck, as seen in `src/pages/common/Profile.tsx`.
**Action:** Pre-compute lookup tables (`classesMap`) using `useMemo` outside of iterators to convert O(M) lookups to O(1), bringing overall render complexity down from O(N*M) to O(N+M).
