...
## 2025-03-03 - [Nested Loop O(N²) Optimization in Classes]
**Learning:** Found a severe performance bottleneck in `Classes.tsx` where `Array.prototype.find()` and `Array.prototype.filter()` over `users` and `students` were executed inside a loop mapping over `classes`. This created O(C * U) and O(C * S) operations during rendering for directors.
**Action:** Replaced the inner `find()` and `filter()` calls with O(1) `Map.prototype.get()` lookups by pre-computing `teachersMap` and `classStudentsMap` in O(U) and O(S) time. Always prefer pre-computing lookup maps outside nested iterations, especially during component rendering.
