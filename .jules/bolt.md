...
**Learning:** Found a performance bottleneck in `TeacherBulletinGrades.tsx` where an `Array.prototype.filter()` over `classComments` was executed inside a `.map()` loop over `classStudents` within the render cycle. This created an O(S*K) operation that impacts rendering performance when a teacher has many students and comments.
**Action:** Replaced the inner `filter()` with an O(1) `Map.prototype.get()` lookup by pre-computing a `studentValidatedCounts` Map in O(K) time using an IIFE before the `.map()` loop. Always pre-compute lookup maps before mapping over arrays in render loops to resolve related items.
