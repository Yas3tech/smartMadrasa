...
## 2026-06-20 - [O(N²) Optimization in Classes]
**Learning:** Found an O(N²) anti-pattern in `Classes.tsx` where `Array.prototype.find()` and `Array.prototype.filter()` were called for each class during rendering. This creates a severe performance bottleneck scaling with classes and users.
**Action:** Used `useMemo` to pre-compute `teachersMap` and `classStudentsMap` (O(U + S)), transforming all subsequent lookups within the render loop into O(1) operations, dropping the render complexity to O(C + U + S).
