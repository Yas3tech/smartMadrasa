...
**Learning:** Found a performance bottleneck in `StudentGradesView.tsx` where an `Array.prototype.find()` lookup over `users` and `courses` was executed inside the `myGrades.map()` render loop. This created an O(G * (U + C)) operation during grade rendering.
**Action:** Replaced the inner `find()` searches with O(1) `Map.prototype.get()` lookups by pre-computing `usersMap` and `coursesMap` (using a composite key `classId::subject`) in O(U + C) time. Always prefer pre-computing lookup maps outside render mapping loops.
