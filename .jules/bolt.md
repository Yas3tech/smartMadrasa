...
## 2026-03-03 - [Nested Array Lookup Optimization in useSchedule]
**Learning:** Found an O(N*M) performance bottleneck in `useSchedule.ts` where `classes.filter(...).map(...)` was executed inside the main `events.filter(...)` loop to determine if an exam belonged to a teacher's assigned classes. This created a severe O(E * C) time complexity issue affecting schedule rendering performance.
**Action:** Replaced the inner array iteration with a pre-computed `Set` of `teacherClassIds` before the loop, enabling O(1) `.has()` lookups. Always extract array transformations outside of filter loops to drop complexity from O(N*M) to O(N+M).
