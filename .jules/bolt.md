...
## 2025-03-03 - [Nested Loop O(C*G) Optimization in Bulletin Generation]
**Learning:** Found a performance bottleneck in both `BulletinPreview.tsx` and `pdfGenerator.ts` where filtering the `grades` array and parsing `Date` objects were executed inside a nested loop mapping over courses. This created an O(C*G) operation during bulletin rendering and PDF generation.
**Action:** Replaced the inner filter with an O(1) `Map.prototype.get()` lookup by pre-computing a `studentPeriodGrades` Map in O(G) time and parsing dates outside the loop. Always pre-compute lookup maps before mapping over arrays to resolve related items.
