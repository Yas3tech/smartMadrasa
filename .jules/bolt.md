...
## 2024-03-30 - Schwartzian Transform for Date Parsing
**Learning:** Instantiating `new Date()` within array iterations (like `.filter` or `.sort`) scales poorly, reaching O(N log N) overhead.
**Action:** Use a Schwartzian transform pattern (`.map(x => ({original: x, val: new Date(x.date).getTime()}))`) to cache parsed values for sorting/filtering, and unwrap afterward (`.map(x => x.original)`). This preserves reference identity and speeds up execution significantly (measured ~6x improvement).
