...
**Learning:** Found a performance bottleneck in both `BulletinPreview.tsx` and `pdfGenerator.ts` where an `Array.prototype.filter()` lookup over `grades` was executed inside a nested loop mapping over courses via `flatMap()`. This created an O(C*G) operation during bulletin rendering and PDF generation.
**Action:** Replaced the inner `filter()` search with an O(1) `Map.prototype.get()` lookup by pre-computing a `gradesByCourse` map in O(G) time. Always pre-compute lookup maps before mapping over arrays to resolve related items.
