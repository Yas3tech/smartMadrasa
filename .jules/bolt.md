...
## 2025-03-03 - [O(C*U) and O(C*S) Optimization in Classes.tsx]
**Learning:** Found a severe performance bottleneck in `Classes.tsx` where `users.find()` and `students.filter()` were executed for each class during rendering. This caused O(C * U) and O(C * S) operations.
**Action:** Replaced the lookups with O(1) `Map` lookups by pre-computing `teachersMap` and `classStudentsMap` using `useMemo`. Always pre-compute lookup maps before mapping over arrays to resolve related items.
