...
## 2025-03-03 - [Nested Loop O(S*K) Optimization in TeacherBulletinGrades]
**Learning:** Found an O(S*K) performance bottleneck in `TeacherBulletinGrades.tsx` where an `Array.prototype.filter()` lookup over `classComments` was executed inside a loop mapping over `classStudents` to compute validation stats. This could cause severe lagging for directors or teachers navigating large classes.
**Action:** Replaced the inner `.filter()` array search with an O(1) `Map.prototype.get()` lookup by pre-computing a `studentValidatedCounts` Map using `useMemo` in O(K) time, significantly improving rendering performance. Always pre-compute map counts outside iteration loops.
