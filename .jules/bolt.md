...
## 2025-03-03 - [Nested Loop O(N*M) Optimization in Messages Component]
**Learning:** Found a severe O(N*M) performance bottleneck in `Messages.tsx` within the `handleSendMessage` function and component render. Calling `Array.prototype.find()` and `Array.prototype.filter()` over large datasets (`allRecipientOptions`, `classes`, `users`) inside a `.map()` loop (iterating over `recipients`) creates blocking operations that degrade UI responsiveness during bulk messaging.
**Action:** Always pre-compute lookup mechanisms using `Map` (for O(1) searches) or grouping (like `studentsByClassMap`) before entering an array `.map()` loop to flatten the complexity and ensure swift execution even with thousands of entries.
