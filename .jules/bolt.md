...
## 2026-03-03 - [Nested Loop O(C*U) and O(C*S) Optimization in Classes]
**Learning:** Found an O(N²) anti-pattern in `Classes.tsx` where `.find()` and `.filter()` operations were executed inside the render loop for each class, resulting in severe performance bottlenecks for directors managing large schools.
**Action:** Pre-computed `teachersMap` and `classStudentsMap` using `useMemo` to eliminate O(C*U) and O(C*S) bottlenecks, dropping the complexity to O(C + U + S) where C, U, S are the lengths of classes, users, and students arrays. Always prefer O(1) hash map lookups over O(N) array iterations inside render loops.
