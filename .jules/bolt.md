...
## 2025-03-03 - [O(N²) Optimization in Messages Recipient Mapping]

**Learning:** Found a performance bottleneck in `Messages.tsx` where an `Array.prototype.find()` lookup over `allRecipientOptions` was executed inside array `.map()` loops when resolving recipient labels and dispatching group messages. This created an O(N * M) operation.
**Action:** Replaced the inner `find()` search with an O(1) `Map.prototype.get()` lookup by pre-computing a `recipientOptionsMap` in an O(M) operation using `useMemo`. Always prefer pre-computing lookup maps before mapping over arrays to resolve related items.
