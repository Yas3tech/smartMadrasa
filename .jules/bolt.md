...
## 2025-03-03 - [Nested Loop O(C*G) Optimization in Bulletin Grades]
**Learning:** Found a severe performance bottleneck in `BulletinPreview.tsx` and `pdfGenerator.ts` where `Array.prototype.filter()` over `grades` (along with slow `new Date()` parsing) was executed inside a nested loop mapping over courses. This created an O(C*G) operation during bulletin rendering and PDF generation.
**Action:** Replaced the inner `filter()` with an O(1) `Map.prototype.get()` lookup by pre-computing a `studentPeriodGradesMap` in O(G) time outside the loop.
