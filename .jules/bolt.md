...
## 2025-03-03 - [O(N^2) Optimization in Messages]
**Learning:** Found an O(N^2) anti-pattern in `Messages.tsx` where `.find()` and `.filter()` operations were executed inside a `.map()` loop during recipient selection and message broadcasting. This creates a performance bottleneck when scaling to many recipients and users.
**Action:** Replaced the inner searches with O(1) Map lookups by pre-computing `recipientOptionsMap`, `classesMap`, `usersMap`, and `studentsByClassMap`.
