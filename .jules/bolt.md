...
## 2024-05-20 - Pre-computed Maps for Array Filtering in Renders
**Learning:** Found an O(S * K) bottleneck in `src/pages/teacher/BulletinGrades.tsx` where `classComments.filter()` was being called inside a `classStudents.map()` render loop.
**Action:** Use `useMemo` to pre-compute entity validations before early returns. This reduces render complexity from O(S * K) to O(S + K), significantly improving large dataset rendering.
