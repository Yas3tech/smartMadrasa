...
## 2026-03-03 - [O(C * (U + S)) Optimization in Classes]
**Learning:** Found a severe render bottleneck in `Classes.tsx` where `users.find()` and `students.filter()` were executed inside a `.map()` loop over classes. This creates an O(C * (U + S)) operation during component rendering.
**Action:** Used `useMemo` to pre-compute `teachersMap` and `studentsByClassMap`, transforming inner lookups to O(1) and overall complexity to O(C + U + S).
