...
## 2026-03-03 - [Optimize array lookups inside render loops]
**Learning:** Components like Profile and TeacherAttendance were calling `.find()` inside `.map()` during renders, causing O(N*M) performance bottlenecks.
**Action:** Use `useMemo` to pre-compute lookup Maps (e.g. `classesMap`) beforehand to ensure O(1) reads inside loops.
