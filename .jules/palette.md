...
## 2025-05-23 - Inline Icon-only Button Accessibility
**Learning:** Icon-only buttons used inline within data tables (like Check for Save, X for Cancel, and Pencil for Edit) are frequently missed during accessibility audits compared to standalone buttons, making critical inline actions inaccessible.
**Action:** Consistently apply `aria-label` attributes and keyboard focus styling (`focus-visible:ring-2 focus-visible:ring-orange-500`) to inline icon-only controls, particularly within list or table items.
