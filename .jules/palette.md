...
## 2024-05-23 - Icon-only buttons in table rows lack accessible names and focus styles

**Learning:** Icon-only action buttons inside table rows (like Edit, Save, Cancel) often lack accessible names (like `aria-label`) and clear visual focus rings, making them inaccessible for screen reader and keyboard users.
**Action:** Always verify icon-only buttons have explicit `aria-label` attributes and use `focus-visible:ring-2` with an appropriate color to provide clear visual feedback for tab-based navigation.
